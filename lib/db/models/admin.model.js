...
/**
 * Before saving a new entry, ensure the password is valid and hash it
 */
...
/**
 * In the case of an update inclusive of a new profile picture, delete the old one.
 */
...
/**
 * Verify access to this account via passphrase
 * @param {String} passphrase
 */
adminSchema.methods.SignIn = function (passphrase) {
	return new Promise(async (resolve, reject) => {
		const same = await compare(passphrase, this.passphrase).catch((err) => {
			reject(err)
		})
